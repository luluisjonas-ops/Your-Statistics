...
export default function Register() {
  ...
  const [form, setForm] = useState({
    ...
    business_type: "Salão de beleza", terms: false,
  });
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      // company is required by backend model; use business_type as placeholder if empty
      await register({ ...payload, company: form.business_type });
        ...
        >
          <div>
            ...
            <p className="mt-4 text-muted-foreground max-w-md">
              É rapidinho: só o essencial. Nossa equipe libera seu acesso após uma análise humana.
            </p>
          ...
          <form onSubmit={submit} className="ys-card p-6 sm:p-8 space-y-5">
            <button type="button" onClick={googleLogin} data-testid="reg-google-btn" className="w-full h-11 rounded-xl border border-border hover:bg-accent text-sm font-medium">
              Continuar com Google
            </button>
            <div className="relative py-1">
              <div className="absolute inset-x-0 top-1/2 h-px bg-border" />
              <div className="relative bg-card w-fit mx-auto px-3 text-xs text-muted-foreground">ou preencha manualmente</div>
            </div>

            <div>
              <label className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Nome completo</label>
              <input required value={form.name} onChange={set("name")} placeholder="Seu nome" data-testid="reg-name"
            ...
            <div>
              ...
              <input required type="email" value={form.email} onChange={set("email")} placeholder="voce@empresa.com" data-testid="reg-email"
                className="mt-1.5 w-full h-11 px-4 rounded-xl border border-border bg-background focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition" />
            </div>
            <div>
              <label className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Tipo de empresa</label>
              <select value={form.business_type} onChange={set("business_type")} data-testid="reg-business-type"
                className="mt-1.5 w-full h-11 px-3 rounded-xl border border-border bg-background focus:outline-none focus:ring-2 focus:ring-primary/30">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                ...
                <div className="relative mt-1.5">
                  <input required type={show ? "text" : "password"} value={form.password} onChange={set("password")} data-testid="reg-password"
                    className="w-full h-11 px-4 pr-11 rounded-xl border border-border bg-background focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition" placeholder="Mínimo 6 caracteres" />
              ...
              <div>
                ...
                <input required type={show ? "text" : "password"} value={form.confirm} onChange={set("confirm")} placeholder="Repita a senha" data-testid="reg-confirm"
                  className="mt-1.5 w-full h-11 px-4 rounded-xl border border-border bg-background focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition" />
              </div>
            </div>
            ...
            <label className="flex items-start gap-3 text-sm text-muted-foreground">
              ...
              <span>Aceito os <a className="underline hover:text-foreground" href="#">Termos de Uso</a> e a <a className="underline hover:text-foreground" href="#">Política de Privacidade</a>.</span>
            </label>
